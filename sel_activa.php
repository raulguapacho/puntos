...
if (isset($_SESSION['s_cedulae'])){
    ...
    $sqlcent="select a.id_entidad, b.clave from clientes a inner join usuarios b on a.id_usuario=b.id_usuario 
              where a.cedula=".$cedulae." and b.activo='0' group by id_entidad";
	...
	$sqlcsub="select id_sube from clientes a inner join usuarios b on a.id_usuario=b.id_usuario where a.cedula=".$cedulae." and b.activo='0'";
...
<h3>Seleccione Entidad y/o Subentidad : </h3>
    <form action="/pcp/" method="post">
	<div id="contenedor">
	<?php if ($entidades==1){ ?>
            <label for="Entidad">Entidad : </label>
            <input disabled type="text" value="<?php echo $rsent['entidad']?>"/>
            <br />
            <input type="hidden" name="entidad" value="<?php echo $rsent['id_entidad']?>"/>
            <br />
            <label for="Subentidad">Subentidad : </label>
            <select name="sube" title="Despliegue para seleccionar una subentidad">
