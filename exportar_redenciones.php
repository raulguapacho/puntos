...
$objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('E1', 'Redención')
                    ->setCellValue('F1', 'Actualizado');

$rol=$_SESSION['s_rol'];

if ($rol==1){
    if (($_SESSION['s_entidade']>0) and ($_SESSION['s_sube']==0) and ($_SESSION['s_cliente']==0)){
        $query="SELECT CONCAT(a.nombre,' ',a.apellidos) as nombre, c.entidad, d.sube, b.points, b.fec_redention, b.actualizado 
        ...
        $query="SELECT CONCAT(a.nombre,' ',a.apellidos) as nombre, c.entidad, d.sube, b.points, b.b.fec_redention, b.actualizado 
                FROM clientes a inner join redenciones b on a.id_cliente=b.id_client
                INNER JOIN entidad c on a.id_entidad=c.id_entidad
                INNER JOIN subentidad d on a.id_sube=d.id_sube
...
while ($fila = mysql_fetch_assoc($puntos)) {
	...
	->setCellValue('E'.$i, $fila['fec_redention'])
	->setCellValue('F'.$i, $fila['actualizado']);
     $i++;
}	

$objPHPExcel->getActiveSheet()->setTitle('redenciones');

header("Content-type: application/vnd.ms-excel; name='excel'");
header("Content-Disposition: filename=redenciones.xlsx");
