...
if ($action == "delete") {
    ...
    $sql_puntos="delete from puntos WHERE id=".$id_puntos;
    $elimpuntos=@mysql_query($sql_puntos) or die (mysql_error());
    
    include('conexionpuntos.php');
    $sql_customer_points="delete from ps_customer_points WHERE id=".$id_puntos;
...
<body>
    ...
    <form method="post" id="frmPuntos" action="/dp/">
        <h2>Seguro desea eliminar los puntos de este usuario?</h2>
        ...
        <input type="text" name="tpun" value="<?php echo $rs_puntos['puntos'];?>" readonly="readonly"/>
        <br />
        <label for="bts">&nbsp;</label>
...
<?php
if (isset($elimpuntos)){
    echo "<script> swal({title: 'Exito', text: 'Puntos del usuario eliminados exitosamente', type: 'success', confirmButtonText: 'Aceptar'});setTimeout ('redireccionar()', 2000)</script>";
}
?>
