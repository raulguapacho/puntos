...
if ($paginar==1){
    ...
    if (($entidad>0) and ($subentidad==0) and ($cliente==0)){
        $query_cli="SELECT COUNT(*) as conteo "
                . "FROM clientes a inner join redenciones b on a.id_cliente=b.id_client "
    ...
    if (($entidad>0) and ($subentidad==0) and ($cliente==0)){
        $query_limit="SELECT b.id, a.cedula, CONCAT (a.nombre,' ',a.apellidos) as nombre, b.points, b.fec_redention "
                    ."FROM clientes a inner join redenciones b on a.id_cliente=b.id_client "
                    ."WHERE a.id_entidad=".$entidad
                    ." LIMIT ".(($page-1)*8).", 8";
    }elseif (($entidad>0) and ($subentidad>0) and ($cliente==0)){
    ...
    if (($entidad>0) and ($subentidad==0) and ($cliente==0)){
        ...
        $query_limit="SELECT b.id, a.cedula, CONCAT (a.nombre,' ',a.apellidos) as nombre, b.puntos, b.fec_redention 
                  FROM clientes a inner join redenciones b on a.id_cliente=b.id_client";
    }
}
...
<body>
    <dav>
        <ul>
            ...
            <li><a><?php echo "Listado de Redenciones por Usuario"; ?></a></li>
        </ul>
    </dav>
    <?php if ($_SESSION['s_rol']==1){?>
<form method="post" id="frmpuntosadmin" action="/lr/">
    <table>
        <tr>
        ...
        ?>
            <tbody
                <tr>
                    ...
                    <td><?php echo $rsCli['fec_redention'];?></td>
...
<table>
    <tr>
	<td>
<form method="post" id="frmPuntos" action="/lr/"/>
<?php
if ($paginar==0){
    $paginar=1;
    echo '<table>
    ...
    $paginar=0;
    if ($page>1){
        echo '<a title="Pagina Inicial" href="/lr/1"><img src="../images/primera.jpg" width="15" height="20" border="0"></a>  |  ';
        echo '<a title="Pagina Anterior" href="/lr/'.($page-1).'"><img src="../images/izq.gif" border="0"></a>  |  ';
    }
    for($i=0; $i<$max_num_paginas;$i++){
        echo '<a href="/lr/'.($i+1).'">'.($i+1).'</a> | ';
    }
    if ($page<$max_num_paginas){
        echo '<a title="Pagina Siguiente" href="/lr/'.($page+1).'"><img src="../images/der.gif" border="0"></a>  |  ';
        echo '<a title="Pagina Final" href="/lr/'.($max_num_paginas).'"><img src="../images/ultima.jpg" width="15" height="20" border="0"></a>  |  ';
    }
    if ($conteo>10){
...
<table>
	...
	<td><a title="Regresar" href="/psa/"><img src="../images/regresar.jpg" width="25" height="30"/></a></td>
        ...
        <?php } ?>
        <td></td>
        <td><a title="Exportar a Excel" href="/exre/"><img src="../images/exportar.gif" width="25" height="20"/></a></td>
    </tr>
</table>
</body>
</html>